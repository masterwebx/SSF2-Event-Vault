...
function Start-Compilation {
    ...
    function Parse-ObjectString($objStr) {
        $props = @{}
        $matches = [regex]::Matches($objStr, '"([^"]+)":\s*([\s\S]*?)(?=,\s*"[^"]+":|$)')
    ...
    foreach ($f in $eventSourceFiles) {
        try {
            ...
            if ($fileContent -match 'eventinfo\s*:\s*Array\s*=\s*\[([^\]]*)\]') {
                $infoStr = $matches[1]
                $msg = "Parsed eventinfo from $f : $infoStr"
                ...
                try {
                    ...
                    $msg = "Error parsing eventInfo: " + $_.Exception.Message
                    if ($LogTextBox) { $LogTextBox.AppendText("$msg`r`n") } elseif (-not $silentMode) { Write-Host $msg }
                    $eventInfo = $null
                }
                $msg = "Parsed eventInfo count: " + $eventInfo.Count + " keys: " + ($eventInfo.Keys -join ',')
                if ($LogTextBox) { $LogTextBox.AppendText("$msg`r`n") } elseif (-not $silentMode) { Write-Host $msg }
                if ($eventInfo -and $eventInfo.Count -gt 0) {
                    if ($eventInfo.ContainsKey("name") -and $eventInfo.ContainsKey("id")) {
    ...
    $msg = "idToNumber mapping: " + ($idToNumber | ConvertTo-Json -Compress)
    if ($LogTextBox) { $LogTextBox.AppendText("$msg`r`n") } elseif (-not $silentMode) { Write-Host $msg }
    ...
    # Create id to number mapping for image renaming
    $msg = "allEventInfos count: " + $allEventInfos.Count
    if ($LogTextBox) { $LogTextBox.AppendText("$msg`r`n") } elseif (-not $silentMode) { Write-Host $msg }
    ...
    $includeArgs = ""
    $linkageNames = @()
    $tempImagesDir = Join-Path $scriptDir "temp_images"
    if (Test-Path $tempImagesDir) { Remove-Item $tempImagesDir -Recurse }
    New-Item -ItemType Directory -Path $tempImagesDir | Out-Null
    if (Test-Path "../compile/images") {
        $originalImageFiles = Get-ChildItem "../compile/images/*.*" | Where-Object { $_.Extension -match '\.(png|jpg|jpeg|gif|bmp|svg)' }
        $msg = "Found $($originalImageFiles.Count) images to process"
        if ($LogTextBox) { $LogTextBox.AppendText("$msg`r`n") } elseif (-not $silentMode) { Write-Host $msg }
        if ($originalImageFiles.Count -gt 0) {
            # Copy and rename images that match event IDs to temp folder with numbered names
            foreach ($img in $originalImageFiles) {
                $base = [System.IO.Path]::GetFileNameWithoutExtension($img.Name)
                if ($idToNumber.ContainsKey($base)) {
                    $number = $idToNumber[$base]
                    $newName = [string]$number + $img.Extension
                    $newPath = Join-Path $tempImagesDir $newName
                    Copy-Item $img.FullName $newPath
                }
            }
            # Use the temp images for compilation
            $imageFiles = Get-ChildItem "$tempImagesDir/*.*" | Where-Object { $_.Extension -match '\.(png|jpg|jpeg|gif|bmp|svg)' }
            ...
            foreach ($img in $imageFiles) {
                ...
                $bitmapPath = $img.FullName
                $embedPath = "../temp_images/" + $img.Name
                $width = 0
                $height = 0
                try {
                    ...
                    # Use 0,0 if can't load
                }
                
                $classContent = "package {`n"
                $classContent += "    import flash.display.BitmapData;`n"
                $classContent += "    `n"
                $classContent += "    [Embed(source=`"$embedPath`")]`n"
                $classContent += "    public dynamic class $linkageName extends BitmapData {`n"
                $classContent += "        public function $linkageName(param1:int = $width, param2:int = $height) {`n"
                $classContent += "            super(param1, param2);`n"
                $classContent += "        }`n"
                $classContent += "    }`n"
                $classContent += "}`n"
    ...
    if ($asFiles.Count -eq 0) {
        ...
        if ($LogTextBox) { $LogTextBox.AppendText("$msg`r`n") } elseif (-not $silentMode) { Write-Host $msg }
                # Build mxmlc arguments now that flags, source paths and main file are known
                if ($linkageNames) {
                    $includeClasses = "-includes=" + ($linkageNames -join ",")
                } else {
                    $includeClasses = ""
                ...
                # Debug: log the linkage names and include string to help diagnose include formatting
                $dbg1 = "LINKAGE ARRAY: " + ($linkageNames -join ",")
                $dbg2 = "INCLUDE STRING: " + $includeClasses
                try { Add-Content -Path (Join-Path $exeDir "history.log") -Value "$(Get-Date): $dbg1" } catch { }
                try { Add-Content -Path (Join-Path $exeDir "history.log") -Value "$(Get-Date): $dbg2" } catch { }
                if ($LogTextBox) { $LogTextBox.AppendText("$dbg1`r`n") ; $LogTextBox.AppendText("$dbg2`r`n") } elseif (-not $silentMode) { Write-Host $dbg1 ; Write-Host $dbg2 }

                $arguments = "-warnings=$warningsFlag -strict=true -source-path=$sourcePathArg -library-path=../aflex_lite/frameworks/libs/framework.swc,../aflex_lite/frameworks/libs/player/32.0/playerglobal.swc $includeClasses `"$relativeMain`" -output=../custom_events.swf"
        ...
        try {
            ...
            # Delete created ActionScript classes for images
            if ($linkageNames) {
                foreach ($name in $linkageNames) {
                    $path = "$sourceDir\$name.as"
                    if (Test-Path $path) {
                        Remove-Item $path
                    }
                }
            ...
            if (Test-Path $tempImagesDir) {
                Remove-Item $tempImagesDir -Recurse
            }
        }
    }
    $log.WriteLine((Get-Date).ToString() + ": Compilation complete")
...
[System.Windows.Controls.Grid]::SetRow($logTextBox, 2)
[System.Windows.Controls.Grid]::SetColumnSpan($logTextBox, 2)
$logTextBox.Width = 880
$logTextBox.Height = 75
$logTextBox.IsReadOnly = $true
$logTextBox.TextWrapping = "Wrap"
$logTextBox.VerticalScrollBarVisibility = "Visible"
$logTextBox.Margin = "10,0,10,10"
...
[System.Windows.Controls.Grid]::SetColumn($sortPanel, 0)
[System.Windows.Controls.Grid]::SetRow($sortPanel, 0)
$sortPanel.Width = 430
$sortPanel.Height = 300
$sortPanel.Margin = "10,10,10,10"
$grid.Children.Add($sortPanel)

# Preview Panel (right side - image only)
$previewPanel = New-Object System.Windows.Controls.Canvas
[System.Windows.Controls.Grid]::SetColumn($previewPanel, 1)
[System.Windows.Controls.Grid]::SetRow($previewPanel, 0)
$previewPanel.Width = 430
$previewPanel.Height = 300
$previewPanel.Margin = "10,10,10,0"
$grid.Children.Add($previewPanel)

# Description Panel (full width above console)
$descriptionPanel = New-Object System.Windows.Controls.Canvas
[System.Windows.Controls.Grid]::SetColumn($descriptionPanel, 0)
[System.Windows.Controls.Grid]::SetRow($descriptionPanel, 1)
[System.Windows.Controls.Grid]::SetColumnSpan($descriptionPanel, 2)
$descriptionPanel.Height = 25
$descriptionPanel.Margin = "10,0,10,0"
    ...
    $checkBox.Add_MouseEnter({
        ...
        if ($index -ge 0) {
            ...
            foreach ($ext in $extensions) {
                $testPath = Join-Path "../compile/images" ($events[$index].Id + $ext)
            ...
            if ($imagePath) {
                try {
                    ...
                    $bitmapImage.UriSource = [System.Uri]::new((Resolve-Path $imagePath).Path, [System.UriKind]::Absolute)
                    $bitmapImage.EndInit()
...
[System.Windows.Controls.Canvas]::SetTop($descriptionBox, 0)
$descriptionBox.Width = 880
...
# Image Preview
$imagePreview = New-Object System.Windows.Controls.Image
[System.Windows.Controls.Canvas]::SetLeft($imagePreview, 0)
[System.Windows.Controls.Canvas]::SetTop($imagePreview, 0)
$imagePreview.Width = 430
$imagePreview.Height = 440
$imagePreview.Stretch = "Uniform"
$previewPanel.Children.Add($imagePreview)

# Select All Button
$selectAllButton = New-Object System.Windows.Controls.Button
$selectAllButton.Content = "Select All"
[System.Windows.Controls.Canvas]::SetLeft($selectAllButton, 0)
[System.Windows.Controls.Canvas]::SetTop($selectAllButton, 230)
...
# Run SSF2 Button
$runSSF2Button = New-Object System.Windows.Controls.Button
$runSSF2Button.Content = "Run SSF2"
[System.Windows.Controls.Canvas]::SetLeft($runSSF2Button, 600)
[System.Windows.Controls.Canvas]::SetTop($runSSF2Button, 10)
$runSSF2Button.Width = 100
$runSSF2Button.Height = 30
$buttonPanel.Children.Add($runSSF2Button)

$runSSF2Button.Add_MouseEnter({ param($sender, $e) $sender.Background = [System.Windows.Media.Brushes]::Yellow })
$runSSF2Button.Add_MouseLeave({ param($sender, $e) $sender.Background = [System.Windows.Media.Brushes]::LightGray })
...
# Event handler for run SSF2 button
$runSSF2Button.Add_Click({
    try {
        $ssf2Path = Join-Path $exeDir "../SSF2.exe"
        if (Test-Path $ssf2Path) {
            Start-Process -FilePath $ssf2Path
            $logTextBox.AppendText("Starting SSF2.exe...`r`n")
        } else {
            $logTextBox.AppendText("ERROR: SSF2.exe not found at $ssf2Path`r`n")
        }
    } catch {
        $logTextBox.AppendText("ERROR: Failed to start SSF2.exe - $($_.Exception.Message)`r`n")
    }
})
...
$runSSF2Button.Background = [System.Windows.Media.Brushes]::LightGray
$runSSF2Button.Foreground = [System.Windows.Media.Brushes]::Black
